				  ...
				  libfat_sector_t s)
{
	...
	fatsect = fs->fat + (fatoffset / fs->bytes_per_sector);
	fsdata = libfat_get_sector(fs, fatsect);
	if (!fsdata)
	    return -1;
	nextcluster = fsdata[fatoffset & (fs->bytes_per_sector - 1)];
	...
	if (!fsdata)
	    return -1;
	nextcluster |= fsdata[fatoffset & (fs->bytes_per_sector - 1)] << 8;
	...
	if (!fsdata)
	    return -1;
	nextcluster =
	    read16((le16_t *) & fsdata[fatoffset & (fs->bytes_per_sector - 1)]);
	...
	if (!fsdata)
	    return -1;
	nextcluster =
	    read32((le32_t *) & fsdata[fatoffset & (fs->bytes_per_sector - 1)]);
