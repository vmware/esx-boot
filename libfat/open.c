/*******************************************************************************
 * Portions Copyright (c) 2011,2016 VMware, Inc.  All rights reserved.
...
libfat_open(int (*readfunc) (intptr_t, void *, size_t, libfat_sector_t),
	    intptr_t readptr, uint16_t bytes_per_sector)
{
    ...
    if (bytes_per_sector != read16(&bs->bsBytesPerSec))
	goto barf;

    if ((bytes_per_sector != LIBFAT_SECTOR_SIZE_512) &&
        (bytes_per_sector != LIBFAT_SECTOR_SIZE_4K))
	goto barf;
    ...
    rootdirsize = ((read16(&bs->bsRootDirEnts) << 5) + bytes_per_sector - 1)
	/ bytes_per_sector;
    ...
    minfatsize = (minfatsize + bytes_per_sector - 1) / bytes_per_sector;
